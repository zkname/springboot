...
import com.zkname.hd.dao.SysUserModuleDAO;
import com.zkname.frame.dao.IBaseDAO;
import com.zkname.hd.entity.SysPlatform;
import com.zkname.hd.entity.SysUserModule;
...
import com.zkname.hd.vo.MenuVo;
import com.zkname.hd.vo.PurviewVo;
...
public class SysUserModuleService extends BaseService<SysUserModule> {
	...
	public MenuVo getMenuTemplate(long platformId){
		...
		mv.setId(0);
//		if(platformId!=-1){
			SysPlatform sysPlatform=sysPlatformService.findById(platformId);
			mv.setName(sysPlatform.getName());
//		}else{
//			mv.setName("所有");
//		}
	...
	private void orderPurviewAndMenu(MenuVo mv,List<Map<String,String>> modules,List<SysUserModulePermission> list){
		...
		for(int i=0;i<modules.size();i++){
			...
			if(parentId.equals(map.get("parentId"))){
				...
				mvSun.setUrl(map.get("resourceValue"));
				if(mvSun.getUrl()!=null){
					mvSun.setUrl(mvSun.getUrl().replaceAll("\\*\\*", ".do"));
				}
				mv.getLists().add(mvSun);
