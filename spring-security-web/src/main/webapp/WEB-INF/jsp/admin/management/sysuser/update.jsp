  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a href="${ctx}/admin/management/sysuser/list">用户管理</a></li>
          ...
          <div class="box box-info">
            ...
            <!-- /.box-header -->
              <div class="box-body">
                ...
                <div class="form-group">
                  <label class="col-sm-2 control-label"><span class="notnull">*</span>用户角色</label>

                  <div class="col-sm-10">
					<c:forEach var="role" items="${roleList }"  varStatus="status">
					  	<c:set var="isChecked" value="false" />
						<c:forEach var="roleSelect" items="${roleSelectList }" varStatus="obj">
							<c:if test="${roleSelect.roleId==role.id }">
								<c:set var="isChecked" value="true" />
								<c:set var="obj.index" value="0" />
							</c:if>
						</c:forEach>
					  	<li><input type="radio" name="roleId" id="roleId" value="${role.id}" class=" <c:if test="${status.index==0}">validate-one-required</c:if> " <c:if test="${isChecked==true}"> checked="checked" </c:if> />${role.name}</li>
					 </c:forEach>
                  <br></div>
