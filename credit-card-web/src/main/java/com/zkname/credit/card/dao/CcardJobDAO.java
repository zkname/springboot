...
public class CcardJobDAO extends BaseDAO<CcardJob> {
	...
	public List<Map<String,Object>> findUserJob(long userId,Date b,Date e){
		String sql="SELECT a.id,CONCAT(c.name,'-',b.name,'[',a.money,'元]') as title,date_format(a.jobDate,'%Y-%m-%d') as start,a.status as status FROM c_card_job as a ,c_card_info as b,c_bank as c where a.creatorId=? and a.jobDate>=? and a.jobDate<? and c.id=a.bankId and b.id=a.cardInfoId order by a.jobDate asc,a.status desc";
