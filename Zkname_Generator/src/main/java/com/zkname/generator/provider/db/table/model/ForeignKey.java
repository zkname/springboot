...
import com.zkname.generator.util.ListHashtable;
...
public class ForeignKey implements java.io.Serializable{
	...
	public ListHashtable getColumnJavas() {
		ListHashtable columns1 = new ListHashtable();
		for(Object aKey:columns.keySet()){
			columns1.put(aKey, StringHelper.uncapitalize(StringHelper.makeAllWordFirstLetterUpperCase(StringHelper.toUnderscoreName(columns.get(aKey).toString()))));
		}
		return columns1;
	}
	
	public ListHashtable getParentColumnJavas() {
		ListHashtable columns1 = new ListHashtable();
		for(Object aKey:parentColumns.keySet()){
			columns1.put(aKey, StringHelper.uncapitalize(StringHelper.makeAllWordFirstLetterUpperCase(StringHelper.toUnderscoreName(parentColumns.get(aKey).toString()))));
		}
		return columns1;
	}
