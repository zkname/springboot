...
public class MemcacheConfiguration {
	...
	private static final Logger log = LoggerFactory.getLogger(MemcacheConfiguration.class);
	...
	public MemcachedClient getDefaultMemcachedClient() throws IOException {
		log.info("init Memcache Configuration ");
		log.info("init Memcache AuthInfo host:{} port:{}",env.getProperty("memecache-0.host"),env.getProperty("memecache-0.port"));
		...
		if(StringUtils.isNoneBlank(env.getProperty("memecache-0.username"),env.getProperty("memecache-0.password"))){
			...
			log.info("init Memcache AuthInfo username:{} password:{}",env.getProperty("memecache-0.username"),env.getProperty("memecache-0.password"));
		}
